...
class BHV_Block:
    ...
    def execute(self, agent: IAgent):
        agent.add_log_text(LoggerLevel.BLOCK, "start blocking")
        if agent.wm.game_mode_type != GameModeType.PlayOn:
            agent.add_log_text(LoggerLevel.BLOCK, f"is not play on {agent.wm.game_mode_type} {GameModeType.PlayOn}")
            return False
        ...
        agent.add_log_text(LoggerLevel.BLOCK, f"reach steps, tm: {teammate_reach_steps} "
        ...
        for unum in range(2, 12):
            agent.add_log_text(LoggerLevel.BLOCK, f">> tm: {unum}")
            if unum not in agent.wm.our_players_dict.keys():
                agent.add_log_text(LoggerLevel.BLOCK, f"### is not in out_player_dict")
                continue
            tm = agent.wm.our_players_dict[unum]

            if tm.uniform_number < 1:
                agent.add_log_text(LoggerLevel.BLOCK, f"### uniform id is less than 1")
                continue
