...
if [ "${is_player}" = "false" -a "${is_coach}" = "false" -a "${is_goalie}" = "false" ]; then
  echo "You must specify at least one of --player, --coach, or --goalie."
  exit 1
fi
